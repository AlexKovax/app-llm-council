...
The `avatar_svg` field stores the personality's avatar. It may hold either
SVG markup (when generated by the LLM) or a `data:` URI (when the user
uploads an image). It is empty when no avatar has been set, in which case the
UI falls back to the personality's initial. Avatar generation/upload is
strictly on demand — creating a personality never triggers an LLM call.
"""
...
# Avatar upload constraints. Uploaded images are stored as base64 data URIs
# in the same `avatar_svg` field. We cap the decoded payload to keep
# personalities.json manageable and reject obviously oversized uploads.
AVATAR_MAX_BYTES = 1_000_000  # 1 MB after base64-decoding
AVATAR_ALLOWED_MIME = {
    "image/png",
    "image/jpeg",
    "image/webp",
...
_DATA_URI_RE = re.compile(
    r"^data:(?P<mime>[a-zA-Z0-9.+/-]+);base64,(?P<data>[A-Za-z0-9+/=]+)$"
)
...
async def create_personality(
    ...
    The personality is persisted with an empty avatar. Avatar generation is
    on demand only — the user must explicitly request it via
    `regenerate_avatar` (LLM) or `upload_avatar` (image file). Until then,
    the UI shows the default initials fallback.
    """
...
def validate_avatar_data_uri(data_uri: str) -> str:
    """Validate a `data:image/...;base64,...` URI and return it cleaned.

    Raises ValueError if the URI is malformed, the MIME type is not allowed,
    or the decoded payload exceeds AVATAR_MAX_BYTES. Returns the original
    URI string on success (it is stored verbatim).
    """
    if not data_uri or not data_uri.strip():
        raise ValueError("Empty avatar data URI")
    match = _DATA_URI_RE.match(data_uri.strip())
    if not match:
        raise ValueError(
            "Invalid avatar format: expected a base64 data URI "
            "(data:<mime>;base64,<...>)"
        )
    mime = match.group("mime").lower()
    if mime not in AVATAR_ALLOWED_MIME:
        raise ValueError(
            f"Unsupported avatar type: {mime}. Allowed: "
            f"{', '.join(sorted(AVATAR_ALLOWED_MIME))}"
        )
    try:
    ...
    if len(decoded) > AVATAR_MAX_BYTES:
        raise ValueError(
            f"Avatar too large: {len(decoded)} bytes "
            f"(max {AVATAR_MAX_BYTES})."
        )
    return data_uri.strip()


def upload_avatar(personality_id: str, data_uri: str) -> Optional[Dict[str, Any]]:
    """Set the avatar for a personality from an uploaded image data URI.

    Validates the URI (format, MIME type, size), then stores it in the
    `avatar_svg` field (which holds either SVG markup or a data URI). Returns
    the updated personality, or None if the personality was not found.
    """
    if get_personality(personality_id) is None:
        return None
    cleaned = validate_avatar_data_uri(data_uri)
    return update_personality(personality_id, avatar_svg=cleaned)
